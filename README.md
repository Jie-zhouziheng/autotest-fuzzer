...
模糊器将：
从 seeds/ 加载初始种子
自动保存触发崩溃的输入到 ouput/test/crashes
运行结束后打印统计摘要并退出
## 运行目标
...
```
模糊器将:识别目标名字TNAME,编译目标到targets/build文件夹，从seeds/$TNAME中获取种子，将崩溃输出到output/crashes/$TNAME，将图表输出到output/plot_data/$TNAME。
